...
public class Genetic implements Closeable
{
	...
	private void crossoverOX(Individual result, Individual parent1, Individual parent2)
	{
		...
		int start = RandomNumbers.nextNumber() % params.nbClients;
		int end = RandomNumbers.nextNumber() % params.nbClients;
		while (end == start)
		{
			end = RandomNumbers.nextNumber() % params.nbClients;
		}
	...
	{
		int nbIterNonProd = 1;
		for (int nbIter = 0 ; (nbIterNonProd <= maxIterNonProd) && ( nanoTime()< timeLimit) ; nbIter++)
		{
			...
			if (!offspring.isFeasible && RandomNumbers.nextNumber() % 2 == 0) // Repair half of the solutions in case of infeasibility
			{
